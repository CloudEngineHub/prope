...
class TransformerEncoder(Module):
    ...
    def __init__(self, cfg: TransformerEncoderConfig) -> None:
        ...
        if cfg.layer.norm_type == "adaLN-Zero":
            ...
            assert (
                cfg.layer.batch_first
            ), "only batch_first=True is supported for adaLN-Zero"
...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, cfg: TransformerEncoderLayerConfig) -> None:
        ...
        if cfg.norm_type == "adaLN-Zero":
            ...
            assert cfg.batch_first, "only batch_first=True is supported for adaLN-Zero"
        ...
        self.linear1 = Linear(
            cfg.d_model, cfg.dim_feedforward, bias=cfg.bias, **factory_kwargs
        )
        self.dropout = Dropout(cfg.dropout)
        self.linear2 = Linear(
            cfg.dim_feedforward, cfg.d_model, bias=cfg.bias, **factory_kwargs
        )
        ...
        if isinstance(cfg.activation, str):
            cfg.activation = _get_activation_fn(cfg.activation)

        # We can't test self.activation in forward() in TorchScript,
        # so stash some information about it instead.
        if cfg.activation is F.relu or isinstance(cfg.activation, torch.nn.ReLU):
            self.activation_relu_or_gelu = 1
        elif cfg.activation is F.gelu or isinstance(cfg.activation, torch.nn.GELU):
...
class TransformerDecoderLayer(Module):
    ...
    def forward(
        ...
        if self.norm_first:
            x = x + self._sa_block(
                self.norm1(x), tgt_mask, tgt_key_padding_mask, tgt_is_causal, sdpa_fn
            )
            x = x + self._mha_block(
                self.norm2(x),
                memory,
            ...
            x = self.norm1(
                x
                + self._sa_block(
                    x, tgt_mask, tgt_key_padding_mask, tgt_is_causal, sdpa_fn
                )
            )
            x = self.norm2(
                x
